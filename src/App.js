...
// 如果havedone中的某个被点击，则在havedone中删除，并添加到later中
  updateLaters(index, item, e){
    ...
    let laters;
    this.setState(state => {
      const n = state.havedones.indexOf(item);
  ...
  render() {
    return (
      <div className="App">
        ...
        />
        <button onClick={this.handleOnClick}>添加</button><br/>
        <span style={{color: "red"}}>{this.state.laters.length}待办</span>
        <span style={{color: "green"}}>{this.state.havedones.length}已完成</span>
